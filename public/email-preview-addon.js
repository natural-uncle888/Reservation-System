(function(){
  function ensureEmailIframe(){
    ...
    if (!host) {
      ...
      if (prev && prev.insertAdjacentElement){
        host = document.createElement('div');
        host.id = 'emailPreviewHost';
        host.className = 'panel';
        host.innerHTML = "<iframe id='emailPreview' sandbox='' style='width:100%;height:70vh;border:0;border-radius:12px;background:#fff;'></iframe>";
  ...
  function renderEmailToRightPane(htmlOrText){
    try{
      ...
      var html = isHtml ? String(htmlOrText || "") :
        "<pre style='white-space:pre-wrap;font-family:ui-monospace,Menlo,monospace;padding:16px;margin:0;'>" +
        String(htmlOrText || "").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;") +
        "</pre>";
  ...
  function buildFromCtx(ctx){
    ...
    var html = "<!doctype html><html><head><meta charset='utf-8'></head><body style='background:#f8fafc;padding:16px;font-family:system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Helvetica,Arial,sans-serif;'><h2 style='margin:0 0 10px;color:#111827'>寄出內容預覽（依 context 組成）</h2>"+section("基本資料",rowsBasic)+section("服務內容",rowsSvc)+section("其他",rowsOther)+"</body></html>";
    return html;
  }
  ...
  window.openDetails = function(item){
    if (typeof orig === "function") {
      try { orig(item); } catch(e){ console.warn("orig openDetails failed", e); }
    }
    try {
      ...
      var ctx = {};
      try {
        ctx = (item && typeof normalizeContext==='function') ? (normalizeContext(item) || {}) : ((item && item.context) || {});
      } catch(e){
        ctx = (item && item.context) || {};
      }
      var iframe = document.getElementById('emailPreview');
      if (!iframe) return;
      if (ctx.email_html_url){
        iframe.removeAttribute('srcdoc');
        iframe.src = ctx.email_html_url;
        return;
      }
      if (ctx.email_html){
        renderEmailToRightPane(ctx.email_html);
        return;
      }
      var raw = (item && item.context && (item.context.raw || item.context.email_text)) ? (item.context.raw || item.context.email_text) : "";
      if (raw){
        renderEmailToRightPane(raw);
        return;
      }
      var autoHtml = buildFromCtx(ctx || {});
      iframe.removeAttribute('src');
      iframe.srcdoc = autoHtml;
    } catch(e){
      console.warn("addon openDetails failed", e);
    }
  };
})();